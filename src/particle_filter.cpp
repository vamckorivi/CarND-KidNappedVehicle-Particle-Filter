...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
    ...
    for(int i= 0; i<observations.size(); i++){
        ...
        //cout<<"obs.id "<<observation.id;
        //cout<<"obs[id] " << observations[i].id;
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
    ...
    for (int i = 0; i < num_particles; i++) {
        ...
        for (unsigned int j = 0; j < transformed_os.size(); j++) {
            ...
            //equations from the particle weight solution from Udacity lesson
            double gaussain = 1/(2*M_PI*s_x*s_y);
            double exponent = pow(pr_x-o_x,2)/(2*pow(s_x, 2)) + (pow(pr_y-o_y,2)/(2*pow(s_y, 2)));
            double obs_w = gaussain * exp( -exponent );
